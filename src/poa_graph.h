...
typedef struct {
    int node_id, rank_id, cumul_len;
    int in_edge_n, in_edge_m, *in_id;
    int out_edge_n, out_edge_m, *out_id;
    int aligned_node_n, aligned_node_m; int *aligned_node_id; // mismatch
    uint8_t base; // 0~m
    // ID, pos ???
} poa_node_t;
...
typedef struct {
    int node_n, node_m; poa_node_t *node; int *rank_to_node_id, **level_to_node_id; // XXX level: for anti-diagnal band width
...
typedef struct {
    ...
    int32_t len, m, *node_id, *query_id; // TODO for INS/DEL: reduce memory
