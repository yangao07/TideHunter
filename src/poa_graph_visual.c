...
int poa_graph_visual(poa_graph_t *graph, char *dot_fn)
{
    ...
    for (i = 0; i < graph->node_n; ++i) {
        ...
        index = i; rank = poa_graph_node_id_to_max_rank(graph, id);
        ...
        if (graph->node[id].aligned_node_n > 0) {
            ...
            if (show_aligned_mismatch) {
                if (i > x_index) {
                    ...
                    for (j = 0; j < graph->node[id].aligned_node_n; ++j) {
                        fprintf(fp, "-> %s ", node_label[graph->node[id].aligned_node_id[j]]);
                        index = poa_graph_node_id_to_index(graph, graph->node[id].aligned_node_id[j]);
