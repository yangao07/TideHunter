...
int abpoa_msa(uint8_t *bseqs, int seq_len, int *pos, int pos_n, char *cons_seq) {
    ...
    for (i = seq_n = 0; i < pos_n-1; ++i) {
        ...
        if (start < 0 || end < 0 || start >= seq_len || end >= seq_len) continue;

        abpoa_cigar_t *abpoa_cigar = 0; int n_cigar = 0;
        abpoa_align_sequence_with_graph(ab, bseqs+start, end-start, abpt, &n_cigar, &abpoa_cigar);

        if (abpt->out_msa) { 
            seq_node_ids_l[seq_n] = 0; seq_node_ids[seq_n] = (int*)malloc((end-start)* sizeof(int));
            abpoa_add_graph_alignment(ab->abg, abpt, bseqs+start, end-start, n_cigar, abpoa_cigar, seq_node_ids[seq_n], seq_node_ids_l+seq_n);
        } else abpoa_add_graph_alignment(ab->abg, abpt, bseqs+start, end-start, n_cigar, abpoa_cigar, NULL, NULL);
